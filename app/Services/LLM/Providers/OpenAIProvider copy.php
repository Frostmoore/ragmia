...
class OpenAIProvider implements Provider {
  public function chat(array $messages, array $opts = []): array {
    ...
    // ✅ Usa Responses API per o3* e gpt-5*
    $isResponses = preg_match('/^(o3|gpt-5)(:|-|$)/i', $model) === 1;

    if ($isResponses) {
        $payload = array_filter([
            'model'             => $model,
            // Responses API: 'input' può essere direttamente l'array {role, content}
            'input'             => array_map(fn($m) => [
                                        'role'    => $m['role'],
                                        'content' => $m['content'],
                                    ], $messages),
            'reasoning'         => ['effort' => $opts['reasoning_effort'] ?? 'medium'],
            'max_output_tokens' => $opts['max_tokens'] ?? null, // ✅ parametro giusto per Responses
        ...
        // ✅ testo robusto per Responses
        $text = trim((string) (data_get($data,'output_text') ?? ''));
        if ($text === '') {
            // fallback: raccogli i blocchi testuali
    ...
    $payload = array_filter([
        'model'       => $model,
        'messages'    => $messages,
        'max_tokens'  => $opts['max_tokens'] ?? null,    // ✅ parametro giusto (prima era max_completion_tokens)
        'temperature' => $opts['temperature'] ?? 1,
    ], fn($v) => !is_null($v));
